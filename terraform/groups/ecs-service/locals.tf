...
locals {
  ...
  service_name                = "suppression-api" # this must match service name
  container_port              = "8080"
  eric_port                   = "10000"
  docker_repo                 = "suppression-api"
  kms_alias                   = "alias/${var.aws_profile}/environment-services-kms"
  lb_listener_rule_priority   = 31 # FIXME
  lb_listener_paths           = ["^/suppressions/.*", "^/suppressions"]
  healthcheck_path            = "/suppressions-api/healthcheck"
  healthcheck_matcher         = "200"
  vpc_name                    = local.stack_secrets["vpc_name"]
  s3_config_bucket            = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename    = "suppression-api.env"
