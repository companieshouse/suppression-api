#!/bin/bash
#
# Start script for suppression-api
...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/suppression-api/env

    PORT="${SUPPRESSIONS_API_PORT}"
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/suppression-api.jar"